...
| `0x0024` | `access.snapshot` | The workspace's access set, republished into the workspace root group so a device that never published one still holds the list it enforces. `ACCESS` answers with the salt and the accepted head's version and digest and never the entries, which are salted hashes the relay stores as opaque bytes, so a joiner had no set at all: it refused every `roster.update` it read and its own claim, and no invited member could appear in a member list (WEALD-L354). A reader adopts a snapshot only when its version and digest equal the head the relay reports, so the head stays authority and a rewritten or replayed one is dropped. Body is the `AccessSet` encoding, unchanged. |
| `0x0025` | `member.photo` | One person's face, as PNG bytes, signed by the device it describes and admitted only when the record names its own author. In the roster family because it is the third thing a device claims about itself, beside its name and its colour; kept out of `roster.update` because that record is read on every fold and image bytes do not belong in it. A cleared photo is this record with no bytes, so a reader that learned a face can unlearn it. Refused above 512 KB. Body is a JSON `MemberPhotoRecord` (`specs/member-photos.md`). |
...
| `accessSetSnapshot` | 17 | `access.snapshot` | `0x0024` |
| `memberPhoto` | 18 | `member.photo` | `0x0025` |
...
| `WAKE` frames per connection per minute | 30 | Every form of a push-handle statement spends two round trips against the relay's small shared pool, the workspace salt read and then the handle row, and the registration ceiling of five per principal per hour is consulted only by `Register`, so `Clear` and `Query` were the last mutating, pool-touching forms with no ceiling at all. Registering, clearing and querying a handle are startup-shaped acts, not a stream: a device states its handle once and states it again only when the ringer re-mints. Charged before the work is deferred, so a refused frame reaches no database, and refused with `quota/rate_limited` on a socket that stays up. Budgeted separately from `KEYS` so a roster prefetch and a handle registration cannot spend each other's allowance (WEALD-L452). |
