...
Three of the four files carry a `note` saying they were generated by the macOS
client's own code rather than by the relay's. That is deliberate and it is the
whole value of them: `hash` is a content address that two implementations must
compute identically, and a Rust test asserting a Rust-generated value would prove
nothing. So the client generates, the relay is checked against the result, and a
change to either encoder is caught by the side that did not change.

The generator is in the private client repository, so you cannot rerun it from
here, and you do not need to: the vectors are checked in, and the relay's own
suites (`backend/wealdrelay/tests/vectors.rs`, `access_vectors.rs`,
`recon_vectors.rs`) hold it to them on every run. For a third implementation these
files are the contract. If one of them is wrong, that is a specification bug and we
