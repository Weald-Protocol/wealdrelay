...
| R2 | Operator is handed plaintext by a downgraded or misconfigured client | Information disclosure | `enc` is a header field the relay reads and enforces against `WEALD_RELAY_MIN_ENC`. Fixed to `mls` on hosted and not configurable there. | 2. Property of the deployment. | `env-denied-plaintext-under-mls-floor`, plus a hosted-profile relay that refuses to start with `none`. |
