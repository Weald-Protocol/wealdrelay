...
| --- | --- | --- |
| `api/openapi.yaml` | OpenAPI 3.1 for the control plane. Roles, step-up, rate limits and declared problem types are structured fields, not sentences. | `specs/backend/hosted-service.md`, `specs/backend/hosted-service.md` |
...
| `registries/audit-events.md` | Every name written to `audit_events`. | `specs/backend/hosted-service.md`, `specs/backend/hosted-service.md` |
| `registries/data-classification.md` | Field-level classification, retention, readership and subprocessor exposure. The table an enterprise review asks for by name. | `specs/backend/hosted-service.md` |
| `state-machines/` | Instance lifecycle, entitlement, domain. Transition tables plus explicit illegal transitions. | `specs/backend/hosted-service.md`, `specs/backend/hosted-service.md`, `specs/backend/hosted-service.md` |
