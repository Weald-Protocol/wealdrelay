...
| `quota/log_budget_exhausted` | quota | The workspace's envelope log is at `WEALD_RELAY_MAX_LOG_GB`. Distinct from `storage_exhausted`, which is the media bucket: the two clear by different levers and a client shown the wrong one would offer the wrong remedy. Media clears by deleting a file; the envelope log clears only by a signed `drop_before` behind an accepted checkpoint, or by a larger plan, so this error names the limit and never an interval. Waiting does not help, and the relay never compacts to make room on its own initiative (`../../relay/lifecycle.md`). | the limit |
