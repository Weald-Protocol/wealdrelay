...
pub fn tree_kind(root: &Path) -> TreeKind {
    // `project.yml` and not `Weald.xcodeproj`. The Xcode project is generated by
    // XcodeGen and is in `.gitignore`, so it exists in a checkout somebody has
    // built in and in no other: a fresh clone, a CI job before its generate step
    // and every `git worktree` of this repository all carry the client and the
    // corpus while carrying no `.xcodeproj`, and this function called every one of
    // them a published mirror. `check_tree` then failed them for holding a corpus
    // they are supposed to hold. `project.yml` is checked in, is the thing that
    // *owns* the Xcode project, and is not in the published mirror, whose contents
    // are `scripts/relay-mirror.py`'s `MIRRORED` roots and nothing else.
...
mod tests {
    ...
    fn a_tree_carrying_the_client_must_carry_the_corpus() {
        ...
        std::fs::write(scratch.path().join("project.yml"), "name: Weald\n").expect("write");
    ...
    /// The regression this marker was changed for: a checkout that carries the
    /// client and the corpus but has never been built in. A `git worktree`, a
    /// fresh clone and a CI job before its `xcodegen generate` step are all this
    /// tree, and all three used to be judged a published mirror and then failed
    /// for carrying the corpus they are supposed to carry.
    #[test]
    fn a_checkout_nobody_has_generated_the_xcode_project_in_is_this_tree() {
        let scratch = tempfile::tempdir().expect("tempdir");
        std::fs::write(scratch.path().join("project.yml"), "name: Weald\n").expect("write");
        assert!(!scratch.path().join("Weald.xcodeproj").exists());
        assert_eq!(tree_kind(scratch.path()), TreeKind::Monorepo);
    }
